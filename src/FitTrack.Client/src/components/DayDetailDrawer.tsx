...
export default function DayDetailDrawer({ planId, date, onClose }: Props) {
  ...
  const completedCount = log?.logEntries.filter(e => e.isCompleted).length ?? 0;
            ...
            <div>
              ...
              {log && (
                <span className={`inline-block mt-1 text-xs font-medium px-2 py-0.5 rounded-full
                  ...
                  {log.dayType === 'Chest' ? 'Chest & Shoulders'
                    : log.dayType === 'Back' ? 'Back & Rear Shoulders'
                    : 'Abs'}
                </span>
              )}
            </div>
            ...
            <>
              ...
              <div className="mb-5">
                <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                  <span>{completedCount} of {totalCount} exercises done</span>
                ...
                {log.logEntries.map(entry => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium ${entry.isCompleted
                        ? 'text-violet-700 line-through' : 'text-gray-800'}`}>
                        {entry.exerciseName}
                      </p>
              ...
              >
                {log.isCompleted ? 'Unmark day as complete' : '✓ Mark day as complete'}
              </button>
            </>
          ) : null}
        </div>
